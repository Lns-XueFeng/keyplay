...
def change_pianowkey(start, end):
    ...
    for r in piano_matrix:
        ...
        for c in r[start:end]:
            if c == " ":
                copy_piano_matrix[index_row][index_col] = "█"
            index_col = index_col + 1
        index_row = index_row + 1
    return copy_piano_matrix


def change_pianobkey(start, end):
    copy_piano_matrix = np.copy(piano_matrix)
    index_row = 0
...
def piano_keydown(key):
    ...
    for k, v in wkey_dict.items():
        if key == k:
            changed_piano_matrix = change_pianowkey(v[0], v[1])
            show_piano(changed_piano_matrix)
            print("\033c", end="")
    ...
    for k, v in bkey_dict.items():
        if key == k:
            changed_piano_matrix = change_pianobkey(v[0], v[1])
            show_piano(changed_piano_matrix)
            print("\033c", end="")
            play_note(key_note.get(key))
